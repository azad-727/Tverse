...
public class PicklistService {
    ...
    public List<PicklistResultDTO> generatePicklist(MultipartFile file, String channel,String brand) throws IOException {
        ...
        if (filename != null && filename.toLowerCase().endsWith(".csv")) {
            ...
            rawOrders = parseCsvOrderFile(file,channel,brand,currentBatchId);
        } else {
            System.out.println("Processing Excel for Analytics: " + filename);
            rawOrders = parseExcelOrderFile(file,channel,brand,currentBatchId);
        }
    ...
    private Map<String,String> getHeaderMapping(String channel){
        ...
        if(Channel.equals("amazon")){
            ...
            map.put("order_status", "Reason for credit entry");
    ...
    private List<OrderRow> parseCsvOrderFile(MultipartFile file,String channel,String brand,String batchId)throws IOException{
        List<OrderRow> list = new ArrayList<>();
        String platform=channel.toUpperCase().trim();
        DataFormatter formatter = new DataFormatter();
        String responseBrand=brand.toUpperCase().trim();
        ...
        try(BufferedReader br = new BufferedReader(new InputStreamReader(file.getInputStream()))){
            ...
            while((line = br.readLine())!=null){
                ...
                if(indices.containsKey("sku") && cols.length > indices.get("sku")){
                    ...
                    if(channel.equalsIgnoreCase("meesho")){
                        if( !("READY_TO_SHIP".equalsIgnoreCase(row.status) || "PENDING".equalsIgnoreCase(row.status)) ){
                            continue;
                        }
                    }
                    if(channel.equalsIgnoreCase("myntra")){
    ...
    private List<OrderRow> parseExcelOrderFile(MultipartFile file, String channel,String brand,String batchId) throws IOException {
        List<OrderRow> list = new ArrayList<>();
        ...
        try (Workbook workbook = new XSSFWorkbook(file.getInputStream())) {
            ...
            for (Row row : sheet) {
                ...
                if (indices.containsKey("sku")) {
                    ...
                    if(channel.equalsIgnoreCase("meesho")){
                        if( !("READY_TO_SHIP".equalsIgnoreCase(orderRow.status) || "PENDING".equalsIgnoreCase(orderRow.status)) ){
                            continue;
                        }
                    }
                    list.add(orderRow);
