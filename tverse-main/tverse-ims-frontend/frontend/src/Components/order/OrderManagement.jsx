...
const OrderManagement = () => {
    ...
    const [stats,setStats] = useState({approved:0,new:0,packed:0,dispatch_ready:0,shipped:0});
    // --- NEW: DRAWER STATE ---
    ...
    const getSlaDisplay = (hours, date) => {
        if (hours < 4) return <div className="sla-badge danger sla-pulse"><i className="bi bi-exclamation-circle-fill"></i><div><div>Urgent: {hours} hrs left</div><div className="small opacity-75">{date}</div></div></div>;
        if (hours < 24) return <div className="sla-badge warning"><i className="bi bi-clock-history"></i><div><div>{hours} hrs left</div><div className="small opacity-75">{date}</div></div></div>;
        return <div className="sla-badge safe"><i className="bi bi-check-circle"></i><div><div>Safe: {hours} hrs</div><div className="small opacity-75">{date}</div></div></div>;
    };
        ...
        <div className="container-fluid p-0 position-relative">
            ...
            <div className="d-flex justify-content-between align-items-end mb-5">
                ...
                <div className="d-flex gap-2">
                    ...
                    <button className="btn btn-success shadow-sm"><i className="bi bi-plus-lg me-2"></i>Create Manual Order</button>
                </div>
            </div>
            ...
            <div className="bg-white rounded-top-4 border border-bottom-0 shadow-sm p-4 pb-0">
                <div className="d-flex justify-content-between align-items-center border-bottom pb-3">
                    <div className="modern-tabs mb-0 border-bottom-0">
                        {['Pending Labels', 'Pending RTD', 'Pending Handover', 'Completed'].map(tab => (
                    ...
                    <div className="d-flex gap-2">
                        <div className="input-group input-group-sm" style={{width: '250px'}}>
                            ...
                            <input type="text" className="form-control" placeholder="Order ID, SKU, Item ID..." />
                        </div>
                        <button className="btn btn-sm btn-outline-dark"><i className="bi bi-filter"></i> Filter</button>
                    </div>
                </div>
            </div>
            ...
            {selectedIds.length > 0 && (
                <div className="floating-dock">
                    ...
                    <button className="dock-btn" title="Hold Shipment"><i className="bi bi-pause-circle fs-5"></i></button>
                    <button className="dock-btn text-danger" title="Cancel Orders"><i className="bi bi-x-circle fs-5"></i></button>
                    <div className="dock-divider"></div>
                    <button className="dock-btn dock-btn-primary" onClick={() => alert(`Generating labels for ${selectedIds.length} orders...`)}><i className="bi bi-printer"></i> Generate Labels</button>
